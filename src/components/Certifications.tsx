      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 lg:gap-6">
          {certifications.map((cert, index) => {
            const CardContent = (
              <div
                className={`bg-card p-6 rounded-2xl shadow-card flex items-center justify-center aspect-square hover:shadow-medium transition-shadow duration-300 animate-fade-in ${cert.link ? 'cursor-pointer' : ''}`}
            ...
            return cert.link ? (
              ...
              <div key={cert.name}>{CardContent}</div>
            );
          })}
        </div>
      </div>
